...
def storeOrderComp(storeOrderList):
    ...
    orderDictWithDates = {}
    storeOrderList = storeOrderList.rename(
        columns={"Article": "SKU", "Delivery Quantity": "Quantity"})
    storeOrderList['Date facture'] = storeOrderList['Date facture'].dt.strftime(
        '%m/%d/%Y')
    ...
    for orderDate in uniqueDates:
        ...
        for key in orderListDict.keys():
            ...
            orderListDict[key] = orderListDict[key].drop(
                ["Ship-to"], axis=1).reset_index().drop(["index"], axis=1)
