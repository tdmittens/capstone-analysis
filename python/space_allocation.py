...
"""
This will use a VBA file created using opensolver to complete integer linear program model
It will save file and create a dataframe using this file
"""
...
def excelApp(specs, inputPath, SpacesPerSKU: int, TotalSpaces: int, TotalSKUs: int):
    ...
    newPath = os.path.realpath(os.path.join(
        os.getcwd(), os.path.dirname(inputPath), os.path.basename(inputPath)))
    ...
    try:
        ...
        try:
            ...
            print(
                "File could not be opened on its second try. The program will now close.")
    ...
    try:
        ExcelApp.Application.Run(os.path.basename(
            inputPath)+"!Module5.setupsolver")
    except:
...
def spaceAllocationDataFrame(inputPath):
    ...
    space_allocation_df.drop(
        space_allocation_df.columns[0:7], axis=1, inplace=True)
    space_allocation_df.drop(
        space_allocation_df.columns[3:5], axis=1, inplace=True)
    space_allocation_df.columns = [
        "SAP #", "Zij", "Number of pick pallets (vi)"]
